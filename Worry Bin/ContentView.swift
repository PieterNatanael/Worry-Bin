...
struct ContentView: View {
    ...
    @State private var worries: [Worry] = []
    @State private var entryButtonSound: AVAudioPlayer?
    ...
    func playClapSound() {
        if let soundURL = Bundle.main.url(forResource: "flute", withExtension: "mp3") {
            do {
                ...
                entryButtonSound = try AVAudioPlayer(contentsOf: soundURL)
                entryButtonSound?.play() // Play the clap sound
            } catch {
                print("Error loading clap sound file: \(error.localizedDescription)")
            }
        } else {
...
/*
//bagus tapi mau ada UI inmprovement
import SwiftUI
import AVFoundation
